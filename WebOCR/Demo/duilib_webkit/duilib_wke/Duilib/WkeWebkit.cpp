...
void CWkeWebkitUI::SetURL( std::wstring strValue)
{
	m_pWindow->m_strUrl = strValue;
	m_pWindow->m_pWebView->loadURL(strValue.c_str());
}

void CWkeWebkitUI::SetFile( std::wstring strValue)
{
	m_pWindow->m_strUrl = strValue;
	m_pWindow->m_pWebView->loadFile(strValue.c_str());
}

std::wstring CWkeWebkitUI::RunJS(std::wstring strValue)
{
...
HWND CWkeWebkitUI::GetWebKitWindow() {
    return m_pWindow == NULL ? NULL : m_pWindow->GetHWND();
}